...
import com.javaclimb.cate.domain.Recommend;
...
import org.apache.mahout.cf.taste.impl.model.GenericDataModel;//用了mahout包里的推荐过滤功能
...
public class CollectController {
    ...
    public Object addCollect(HttpServletRequest request) {
        ...
        String type = request.getParameter("type");               //收藏类型（0美食1美食合集）
        ...
        if (foodId == null || foodId.equals("")) {
            ...
            jsonObject.put(Consts.MSG, "收藏美食为空");
            return jsonObject;
        }
        if (collectService.existFoodId(Integer.parseInt(userId), Integer.parseInt(foodId))) {
        ...
        Collect.setFoodId(Integer.parseInt(foodId));
    ...
    public Object deleteCollect(HttpServletRequest request) {
        String userId = request.getParameter("userId");           //用户id
        String foodId = request.getParameter("foodId");           //美食id
        boolean flag = collectService.deleteByUserIdFoodId(Integer.parseInt(userId), Integer.parseInt(foodId));
        return flag;
    }
    ...
    public Object topRecommendOfUserId(HttpServletRequest request) {
        ...
        if (userId != 0l) {
            ...
            for (Collect col : list) {
                ...
                r.setFoodId(col.getFoodId().longValue());
                r.setCommend(1);
                allRecommend.add(r);
            }
            ...
            if (cfItemIds != null) {
                ...
                for (Long l : cfItemIds) {
                    Food s = foodService.selectByPrimaryKey(l.intValue());
                    ls.add(s);
                }
                return ls;
            }
    ...
    public DataModel getDadaModel(List<Recommend> allRecommend) {
        ...
        if (allRecommend != null && allRecommend.size() > 0) {
            // 用户id当key，用户收藏的美食id列表当value，存到map里
            ...
            for (Recommend rItem : allRecommend) {
                ...
                Long foodId = rItem.getFoodId();
                Integer commend = rItem.getCommend();
                Integer preference = 0;
                if (commend != 0) {
                    preference = commend;
                    preferenceList.add(new GenericPreference(userId, foodId, preference));
            ...
            System.out.println("******数据库中没有任何美食收藏记录！******");
            return null;
        }
    }
